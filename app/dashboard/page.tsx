...
export default function DashboardPage() {
    ...
    const { loadDashboard, sites, usage, teamMembersCount, isLoading } = useDashboardStore()
    ...
    useEffect(() => {
        if (tenant) {
            loadDashboard(tenant.id, tenant.plan?.monthly_queries || 0)
        }
    ...
    return (
        <div className="space-y-10">
            <h1 className="text-4xl font-bold">Welcome back, {user.user_metadata?.name || user.email}</h1>
            <DashboardStats
                sites={sites}
                usage={usage}
                planLimit={tenant.plan?.monthly_queries}
